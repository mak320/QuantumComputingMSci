...
for i, s in enumerate([1, 10, 100, 1000]):
    ...
    for Nx in Nx_arr:
        for Nv in Nv_arr:
            vmax = (Nv-1) * dv /2 
            F1 = create_F1_matrix() + create_F1_bar(s)
    ...
    plt.hist(eigenvals, histtype="stepfilled", edgecolor=colors[i], alpha=0.2, color=colors[i], label=r"z = {}".format(s))
